...
class SqlStatements
{
    ...
    public static function getUtilityClassName($magentoEdition, $magentoVersion)
    {
        ...
        $utilClassName = sprintf('TechDivision\Import\Product\Bundle\Utils\%s\V%s\SqlStatements', ucfirst($magentoEdition), $magentoVersion);

        // if NOT available, use the default utility class name
        if (!class_exists($utilClassName)) {
            // prepare the Magento edition/version specific utility classname
            if (!class_exists($utilClassName = sprintf('TechDivision\Import\Product\Bundle\Utils\%s\SqlStatements', ucfirst($magentoEdition)))) {
